...
public class AsyncCommandPlugin extends DefaultMetaDataPlugin {
	...
	/** ノード名 */
	private static final String NODE_NAME = "AsyncCommand";

	/** ノードアイコン */
	private static final String NODE_ICON = "script_go.png";
	...
	protected String nodeDisplayName() {
		return AdminClientMessageUtil.getString("ui_metadata_asyncCommand_AsyncCommandPluginManager_asyncCommand");
	}
	...
	protected String definitionClassName() {
		return AsyncCommandDefinition.class.getName();
	}

	@Override
	protected void itemCreateAction(String folderPath) {
		CreateAsyncCommandDialog dialog = new CreateAsyncCommandDialog(definitionClassName(), nodeDisplayName(), folderPath, false);
	...
	protected void itemCopyAction(MetaDataItemMenuTreeNode itemNode) {
		CreateAsyncCommandDialog dialog = new CreateAsyncCommandDialog(definitionClassName(), nodeDisplayName(), "", true);
		dialog.setNamePolicy(isPathSlash(), isNameAcceptPeriod());
		dialog.addDataChangeHandler(new DataChangedHandler() {
			@Override
			public void onDataChanged(DataChangedEvent event) {
	...
	protected MetaDataMainEditPane workSpaceContents(MetaDataItemMenuTreeNode itemNode) {
		return new AsyncCommandEditPane(itemNode, this);
	}
