...
public class BulkDropLayout extends EntityViewItemDropLayout {
	 ...
	 * コンストラクタ
	 */
	public BulkDropLayout(String defName) {
		this(1, defName);
	}
	 ...
	 * @param numColumns
	 */
	public BulkDropLayout(int numColumns, String defName) {
		super(numColumns);
			...
			if (dragTarget instanceof ListGrid) {
				if (EntityViewDragPane.DRAG_TYPE_SECTION.equals(dragTarget.getDragType())) {
					...
					if (VirtualPropertyItem.class.getName().equals(name)) {
						...
						ElementControl newElement = new ElementControl(defName, FieldReferenceType.BULK, record);
