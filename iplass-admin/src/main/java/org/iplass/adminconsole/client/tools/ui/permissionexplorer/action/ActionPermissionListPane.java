...
import org.iplass.adminconsole.client.tools.data.permissionexplorer.ActionPermissionInfoDS;
...
/**
 * ActionPermissionListパネル
 */
public class ActionPermissionListPane extends PermissionListPane {
	 ...
	 */
	public ActionPermissionListPane(ActionPermissionMainPane mainPane, List<Entity> roleList) {
	...
	protected void doRefreshGrid() {
		...
		ds = ActionPermissionInfoDS.getInstance(getRoleMap());
			...
			public void showRolePermissionEditDialog(final PermissionTreeNode record, final String defName, final String roleCode, final int roleIndex, final PermissionEditDialog dialog) {
				...
				if (permissionArray != null) {
					for (Entity permission : permissionArray) {
						//ここが個別
						PermissionEditPane permissionEditPane = new ActionPermissionEditPane(defName, getRoleEntity(roleCode), permission);
						dialog.setPermissionEditPane(permissionEditPane);
					}
				} else {
					//ここが個別
					PermissionEditPane permissionEditPane = new ActionPermissionEditPane(defName, getRoleEntity(roleCode), null);
					dialog.setPermissionEditPane(permissionEditPane);
				}
	...
	protected void update() {
		...
		if (updateList.isEmpty()) {
			...
			//ここが個別
			service.updateActionPermissionData(TenantInfoHolder.getId(), updateList, new AsyncCallback<Void>() {
