...
public class EqlWorksheetPlugin extends DefaultAdminPlugin {

	/** ノード表示名 */
	private static final String NODE_NAME = "EQLWorksheet";

	/** ノードタイプ */
	private static final String NODE_TYPE = "EQLWorksheet";

	/** ノードアイコン */
	private static final String NODE_ICON = "page_white_database.png";
	...
	public void onNodeContextClick(final AdminMenuTreeNode node) {
		if (NODE_TYPE.equals(node.getType())){
			...
			if (contextMenu == null) {
				...
				MenuItem execMenuItem = new MenuItem(AdminClientMessageUtil.getString("ui_tools_eql_EqlWorksheetPluginManager_startEqlWorksheet"), NODE_ICON);
	...
	private void addTab(AdminMenuTreeNode node) {
		tabCount++;
		workspace.addTab(node.getName(), node.getIcon(), String.valueOf(tabCount), new EqlWorksheetMainPane(tabCount));
	}
