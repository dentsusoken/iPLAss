...
public class PrefsPlugin extends DefaultMetaDataPlugin {
	...
	/** ノード名 */
	private static final String NODE_NAME = "Preference";

	/** ノードアイコン */
	private static final String NODE_ICON = "tooloptions.png";
	...
	protected String nodeDisplayName() {
		return AdminClientMessageUtil.getString("ui_metadata_prefs_PrefsPluginManager_preference");
	}
	...
	protected void itemCreateAction(String folderPath) {
		// Preference作成
		CreatePrefsDialog dialog = new CreatePrefsDialog(definitionClassName(), nodeDisplayName(), folderPath, false);
	...
	protected void itemCopyAction(MetaDataItemMenuTreeNode itemNode) {
		// 新規Preference作成
		CreatePrefsDialog dialog = new CreatePrefsDialog(definitionClassName(), nodeDisplayName(), "", true);
		dialog.setNamePolicy(isPathSlash(), isNameAcceptPeriod());
		dialog.addDataChangeHandler(new DataChangedHandler() {
			@Override
			public void onDataChanged(DataChangedEvent event) {
	...
	protected MetaDataMainEditPane workSpaceContents(MetaDataItemMenuTreeNode itemNode) {
		return new PrefsEditPane(itemNode, this);
	}
