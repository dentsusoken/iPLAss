...
public class MessagePlugin extends DefaultMetaDataPlugin {
	...
	/** ノード名 */
	private static final String NODE_NAME = "Message";

	/** ノードアイコン */
	private static final String NODE_ICON = "message.png";
	...
	protected String nodeDisplayName() {
		return AdminClientMessageUtil.getString("ui_metadata_message_MessagePluginManager_messageCategory");
	}
	...
	protected String definitionClassName() {
		return MessageCategory.class.getName();
	}

	@Override
	protected void itemCreateAction(String folderPath) {
		// 新規メッセージカテゴリ作成
		CreateMessageCategoryDialog dialog = new CreateMessageCategoryDialog(definitionClassName(), nodeDisplayName(), folderPath, false);
	...
	protected void itemCopyAction(MetaDataItemMenuTreeNode itemNode) {
		// 新規メニューツリー作成
		CreateMessageCategoryDialog dialog = new CreateMessageCategoryDialog(definitionClassName(), nodeDisplayName(), "", true);
		dialog.setNamePolicy(isPathSlash(), isNameAcceptPeriod());
		dialog.addDataChangeHandler(new DataChangedHandler() {
			@Override
			public void onDataChanged(DataChangedEvent event) {
	...
	protected MetaDataMainEditPane workSpaceContents(MetaDataItemMenuTreeNode itemNode) {
		return new MessageCategoryEditPane(itemNode, this);
	}
