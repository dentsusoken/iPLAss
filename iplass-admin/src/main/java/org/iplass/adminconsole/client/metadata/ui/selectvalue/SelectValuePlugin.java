...
public class SelectValuePlugin extends DefaultMetaDataPlugin {
	...
	/** ノード名 */
	private static final String NODE_NAME = "SelectValue";

	/** ノード表示名 */
	private static final String NODE_DISPLAY_NAME = "SelectValue";

	/** ノードアイコン */
	private static final String NODE_ICON = "configure.png";
	...
	protected String definitionClassName() {
		return SelectValueDefinition.class.getName();
	}

	@Override
	protected void itemCreateAction(String folderPath) {
		CreateSelectValueDialog dialog = new CreateSelectValueDialog(definitionClassName(), nodeDisplayName(), folderPath, false);
	...
	protected void itemCopyAction(MetaDataItemMenuTreeNode itemNode) {
		CreateSelectValueDialog dialog = new CreateSelectValueDialog(definitionClassName(), nodeDisplayName(), "", true);
		dialog.setNamePolicy(isPathSlash(), isNameAcceptPeriod());
		dialog.addDataChangeHandler(new DataChangedHandler() {
			@Override
			public void onDataChanged(DataChangedEvent event) {
	...
	protected MetaDataMainEditPane workSpaceContents(MetaDataItemMenuTreeNode itemNode) {
		return new SelectValueEditPane(itemNode, this);
	}
