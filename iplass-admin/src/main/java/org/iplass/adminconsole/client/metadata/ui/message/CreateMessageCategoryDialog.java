 ...
 * メッセージカテゴリ編集ダイアログ
 */
public class CreateMessageCategoryDialog extends MetaDataCreateDialog {

	public CreateMessageCategoryDialog(String definitionClassName, String nodeDisplayName, String folderPath, boolean isCopyMode) {
	...
	protected void saveAction(final SaveInfo saveInfo, final boolean isCopyMode) {
		...
		checkExist(saveInfo.getName(), new Callable<Void>() {
			...
			@Override
			public Void call() {
				createMessageCategory(saveInfo, isCopyMode);
				return null;
			}
		});
	}
	 ...
	 * 登録処理
	 */
	private void createMessageCategory(final SaveInfo saveInfo, final boolean isCopyMode) {
