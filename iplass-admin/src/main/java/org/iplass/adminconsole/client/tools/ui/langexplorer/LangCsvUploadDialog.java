 ...
 */
public class LangCsvUploadDialog extends AbstractWindow {
	private static final String RESOURCE_PREFIX = "ui_tools_langexplorer_LangCsvUploadDialog_";
	private static final String UPLOAD_SERVICE = "service/langupload";
	 ...
	 * コンストラクタ
	 */
	public LangCsvUploadDialog(final OutputMode mode, final String definitionName, final String path) {

		setWidth(700);
		setMinWidth(500);
		setHeight(500);
		setMinHeight(400);
		setTitle("Upload Language Csv Data");
		...
		//Input Items
		//------------------------
		Label title = new Label(AdminClientMessageUtil.getString("ui_tools_langexplorer_LangCsvUploadDialog_importLangCsvFile"));
		...
		upload.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				uploader.add(new Hidden("mode", mode.name()));
				uploader.add(new Hidden("definitionName", definitionName));//modeがMULTIの場合null
				uploader.add(new Hidden("path", path));//modeがMULTIの場合null
				uploader.submit();
			}
		});
	 ...
	 * @param handler
	 */
	public void addDialogCloseHandler(DataChangedHandler handler) {
