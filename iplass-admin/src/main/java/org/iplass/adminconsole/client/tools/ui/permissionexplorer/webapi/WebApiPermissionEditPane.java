...
public class WebApiPermissionEditPane extends PermissionEditPane {
	...
	private TextItem txtTargetWebApiField;
	 ...
	 * コンストラクタ
	 *
	 * @param target 対象WebAPI
	 * @param roleEntity 対象ロールEntity
	 * @param permissionEntity 対象PermissionEntity(新規時はnull)
	 */
	public WebApiPermissionEditPane(String target, Entity roleEntity, Entity permissionEntity) {
		...
		txtTargetWebApiField = new TextItem("targetWebApi", "Target WebApi");
		txtTargetWebApiField.setCanEdit(false);
		txtTargetWebApiField.setDisabled(true);
		txtTargetWebApiField.setWidth(370);
		...
		permissionTargetForm.setItems(txtNameField, areaDescriptionField, txtTargetWebApiField, txtRoleField);
	...
	private void initializeData(final String target, final Entity roleEntity, final Entity permissionEntity) {
		if (permissionEntity == null) {
			...
			//ここが個別
			editEntity.setValue("targetWebApi", target);
			editEntity.setValue("role", roleEntity);
			editEntity.setDefinitionName("mtp.auth.WebApiPermission");
		} else {
	...
	private void createFieldData() {
		...
		txtTargetWebApiField.setValue(editEntity.getValueAs(String.class, "targetWebApi"));
