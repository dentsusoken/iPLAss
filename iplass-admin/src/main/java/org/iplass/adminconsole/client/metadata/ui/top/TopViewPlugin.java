 ...
 * @author lis3wg
 */
public class TopViewPlugin extends DefaultMetaDataPlugin {

	/** カテゴリ名 */
	private static final String CATEGORY_NAME = MetaDataConstants.META_CATEGORY_VIEW_COMPONENTS;

	/** ノード名 */
	private static final String NODE_NAME = "TopView";

	/** ノードアイコン（TreeGridの場合[SKIN]だと「TreeGrid」が含まれるため[SKINIMG]を使用）  */
	private static final String NODE_ICON = "layout_content.png";
	...
	protected String nodeDisplayName() {
		return AdminClientMessageUtil.getString("ui_metadata_top_TopViewPluginManager_topView");
	}
	...
	protected String definitionClassName() {
		return TopViewDefinition.class.getName();
	}

	@Override
	protected void itemCreateAction(String folderPath) {
		// 新規作成
		CreateTopViewDialog dialog = new CreateTopViewDialog(definitionClassName(), nodeDisplayName(), folderPath, false);
	...
	protected void itemCopyAction(MetaDataItemMenuTreeNode itemNode) {
		// 新規作成
		CreateTopViewDialog dialog = new CreateTopViewDialog(definitionClassName(), nodeDisplayName(), "", true);
		dialog.setNamePolicy(isPathSlash(), isNameAcceptPeriod());
		dialog.addDataChangeHandler(new DataChangedHandler() {
			@Override
			public void onDataChanged(DataChangedEvent event) {
	...
	protected MetaDataMainEditPane workSpaceContents(MetaDataItemMenuTreeNode itemNode) {
		return new TopViewEditPane(itemNode, this);
	}
