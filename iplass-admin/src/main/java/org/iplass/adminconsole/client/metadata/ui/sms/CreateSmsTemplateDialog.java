...
public class CreateSmsTemplateDialog extends MetaDataCreateDialog {

	public CreateSmsTemplateDialog(String definitionClassName, String nodeDisplayName, String folderPath, boolean isCopyMode) {
	...
	protected void saveAction(final SaveInfo saveInfo, final boolean isCopyMode) {
		...
		checkExist(saveInfo.getName(), new Callable<Void>() {
			...
			@Override
			public Void call() {
				createSmsTemplate(saveInfo, isCopyMode);
				return null;
			}
		});
	}

	private void createSmsTemplate(final SaveInfo saveInfo, final boolean isCopyMode) {
		...
		if (isCopyMode) {
			...
			SmsMailTemplateDefinition definition = new SmsMailTemplateDefinition();
