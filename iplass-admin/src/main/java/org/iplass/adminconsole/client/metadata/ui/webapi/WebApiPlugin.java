...
public class WebApiPlugin extends DefaultMetaDataPlugin {

	/** カテゴリ名 */
	private static final String CATEGORY_NAME = MetaDataConstants.META_CATEGORY_CUSTOMIZATION;

	/** ノード名 */
	private static final String NODE_NAME = "WebApi";

	/** ノード表示名 */
	private static final String NODE_DISPLAY_NAME = "WebApi";

	/** ノードアイコン */
	private static final String NODE_ICON = "server_lightning.png";
	...
	protected String definitionClassName() {
		return WebApiDefinition.class.getName();
	}

	@Override
	protected void itemCreateAction(String folderPath) {
		CreateWebApiDialog dialog = new CreateWebApiDialog(definitionClassName(), nodeDisplayName(), folderPath, false);
	...
	protected void itemCopyAction(MetaDataItemMenuTreeNode itemNode) {
		CreateWebApiDialog dialog = new CreateWebApiDialog(definitionClassName(), nodeDisplayName(), "", true);
		dialog.setNamePolicy(isPathSlash(), isNameAcceptPeriod());
		dialog.addDataChangeHandler(new DataChangedHandler() {
			@Override
			public void onDataChanged(DataChangedEvent event) {
	...
	protected MetaDataMainEditPane workSpaceContents(MetaDataItemMenuTreeNode itemNode) {
		return new WebApiEditPane(itemNode, this);
	}
