...
public class OpenApiSupportPlugin extends DefaultAdminPlugin {
	...
	public void onNodeContextClick(final AdminMenuTreeNode node) {
		if (NODE_TYPE.equals(node.getType())) {
			if (contextMenu == null) {
				// FIXME ラベルを変更する
				//				String menuItemLabel = AdminClientMessageUtil.getString("ui_tools_metaexplorer_MetaDataExplorerPluginManager_startMetaDataExplorer");
				String menuItemLabel = "OpenApi(Swagger)Supportを起動する";
	...
	private void addOpenApiSupportTab() {
		...
		// TODO タブ名に括弧が含まれる場合、タブクローズ時にエラーになる。（onFocus にタブIDを指定したメソッドが用意されており、 () が存在することでメソッドコールになってしまうと考えられる）
