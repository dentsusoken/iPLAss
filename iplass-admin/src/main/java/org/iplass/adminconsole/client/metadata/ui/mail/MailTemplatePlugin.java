...
public class MailTemplatePlugin extends DefaultMetaDataPlugin {
	...
	protected String nodeDisplayName() {
		return AdminClientMessageUtil.getString("ui_metadata_mail_MailTemplatePluginManager_mailTemplate");
	}
	...
	protected String definitionClassName() {
		return MailTemplateDefinition.class.getName();
	}

	@Override
	protected void itemCreateAction(String folderPath) {
		CreateMailTemplateDialog dialog = new CreateMailTemplateDialog(definitionClassName(), nodeDisplayName(), folderPath, false);
	...
	protected void itemCopyAction(MetaDataItemMenuTreeNode itemNode) {
		CreateMailTemplateDialog dialog = new CreateMailTemplateDialog(definitionClassName(), nodeDisplayName(), "", true);
		dialog.setNamePolicy(isPathSlash(), isNameAcceptPeriod());
		dialog.addDataChangeHandler(new DataChangedHandler() {
			@Override
			public void onDataChanged(DataChangedEvent event) {
	...
	protected MetaDataMainEditPane workSpaceContents(MetaDataItemMenuTreeNode itemNode) {
		return new MailTemplateEditPane(itemNode, this);
	}
