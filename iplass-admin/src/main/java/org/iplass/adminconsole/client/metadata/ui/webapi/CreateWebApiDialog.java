...
public class CreateWebApiDialog extends MetaDataCreateDialog {

	public CreateWebApiDialog(String definitionClassName, String nodeDisplayName, String folderPath, boolean isCopyMode) {
	...
	protected void saveAction(final SaveInfo saveInfo, final boolean isCopyMode) {
		...
		checkExist(saveInfo.getName(), new Callable<Void>() {
			...
			@Override
			public Void call() {
				createWebAPI(saveInfo, isCopyMode);
				return null;
			}
		});
	}

	private void createWebAPI(final SaveInfo saveInfo, final boolean isCopyMode) {
