...
public class PackagePlugin extends DefaultAdminPlugin {

	/** ノード表示名 */
	private static final String NODE_NAME = "Packaging";

	/** ノードタイプ  */
	private static final String NODE_TYPE = "Packaging";

	/** ノードアイコン（TreeGridの場合[SKIN]だと「TreeGrid」が含まれるため[SKINIMG]を使用）  */
	private static final String NODE_ICON = "pack/basket.png";
	...
	public void onNodeContextClick(final AdminMenuTreeNode node) {
		if (NODE_TYPE.equals(node.getType())){
			...
			if (contextMenu == null) {
				...
				MenuItem execMenuItem = new MenuItem(AdminClientMessageUtil.getString("ui_tools_pack_PackagingPluginManager_startPackaging"), NODE_ICON);
	...
	public PackageMainPane addTab() {
		if (mainPane == null || !workspace.existTab(NODE_NAME, NODE_NAME)) {
			mainPane = new PackageMainPane(this);
		}
