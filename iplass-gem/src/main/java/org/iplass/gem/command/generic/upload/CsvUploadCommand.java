...
public final class CsvUploadCommand extends DetailCommandBase {
	...
	public String execute(final RequestContext request) {
		...
		Set<String> csvUploadProperties = form.getCondSection().getCsvUploadPropertiesSet();
		...
		if (gcs.isCsvUploadAsync()) {
			...
			try (InputStream is = file.getInputStream()){
				service.asyncUpload(is, file.getFileName(), defName, viewName, uniqueKey, isDenyInsert, isDenyUpdate, isDenyDelete, csvUploadProperties,
			...
			try (InputStream is = file.getInputStream()){
				CsvUploadStatus result = service.upload(is, defName, uniqueKey, isDenyInsert, isDenyUpdate, isDenyDelete, csvUploadProperties,
						toTransactionType(csvUploadTransactionType), commitLimit, gcs.isCsvDownloadReferenceVersion(), form.isDeleteSpecificVersion());
