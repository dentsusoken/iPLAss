...
@ActionMapping(
	...
	result={
		...
		@Result(status=Constants.CMD_EXEC_SUCCESS_ASYNC, type=Type.JSP,
				value=Constants.CMD_RSLT_JSP_CSV_UPLOAD,
				templateName="gem/generic/upload/csvUpload",
				layoutActionName=Constants.LAYOUT_NORMAL_ACTION),
		@Result(status=Constants.CMD_EXEC_FAILURE, type=Type.JSP,
				value=Constants.CMD_RSLT_JSP_CSV_UPLOAD,
				templateName="gem/generic/upload/csvUpload",
				layoutActionName=Constants.LAYOUT_NORMAL_ACTION),
		...
		if (gcs.isCsvUploadAsync()) {
			...
			try (InputStream is = file.getInputStream()){
				service.asyncUpload(is, file.getFileName(), defName, viewName, uniqueKey,
						toTransactionType(csvUploadTransactionType), commitLimit, gcs.isCsvDownloadReferenceVersion());
