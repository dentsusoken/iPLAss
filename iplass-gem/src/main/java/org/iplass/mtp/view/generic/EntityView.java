...
public class EntityView implements Definition {
	...
	public List<FormView> getViews() {
		if (this.views == null) this.views = new ArrayList<FormView>();
		return views;
	}
...
//					if (name == null) {
	...
	public boolean isAutoGenerateDetailView(String viewName) {
		List<ViewControlSetting> list = getViewControlSettings().stream()
				.filter(s -> viewName.equals(s.getName()))
				.filter(s -> s.isAutoGenerateDetailView())
	...
	public boolean isAutoGenerateSearchView(String viewName) {
		List<ViewControlSetting> list = getViewControlSettings().stream()
				.filter(s -> viewName.equals(s.getName()))
				.filter(s -> s.isAutoGenerateSearchView())
				.collect(Collectors.toList());
	...
	public boolean isAutoGenerateBulkView(String viewName) {
		List<ViewControlSetting> list = getViewControlSettings().stream()
				.filter(s -> viewName.equals(s.getName()))
				.filter(s -> s.isAutoGenerateBulkView())
				.collect(Collectors.toList());
