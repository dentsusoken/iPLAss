...
<%
					...
					selector = "textarea";
%>
<textarea class="form-size-05 inpbr" style="max-width: 100%; <c:out value="<%=customStyle%>"/>" rows="5" cols="30"></textarea>
<%
...
<%
							...
							//テキストエリア
%>
<textarea name="<c:out value="<%=propName %>"/>" class="form-size-05 inpbr" style="max-width: 100%; <c:out value="<%=customStyle%>"/>" rows="5" cols="30"><c:out value="<%=str %>"/></textarea>
<%
						} else if (editor.getDisplayType() == StringDisplayType.RICHTEXT) {
							//リッチテキスト
...
<%
					...
					//テキストエリア
%>
<textarea name="<c:out value="<%=propName%>"/>" rows="5" cols="30" class="form-size-05 inpbr" style="max-width: 100%; <c:out value="<%=customStyle%>"/>"><c:out value="<%=str %>"/></textarea>
<%
				} else if (editor.getDisplayType() == StringDisplayType.RICHTEXT) {
					//リッチテキスト
					request.setAttribute(Constants.EDITOR_RICHTEXT_LIBRARY, editor.getRichTextLibrary());
