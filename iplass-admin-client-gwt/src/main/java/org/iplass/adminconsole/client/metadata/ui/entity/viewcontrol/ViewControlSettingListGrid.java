...
//		permitRolesField.setWidth(250);
		...
		// 各フィールドをListGridに設定
		setFields(displayNameField, definedDvField, autogenerateDvField, definedSvField, autogenerateSvField, definedBvField, autogenerateBvField,
				permitRolesField);
	...
	public void setDefinition(List<ViewControlSetting> settings, List<String> detailViewNames, List<String> searchViewNames,
		...
		for (ViewControlSetting setting : settings) {
			...
			if (setting.getPermitRoles() != null && !setting.getPermitRoles()
					.isEmpty()) {
		...
		List<ViewControlSettingListGridRecord> records = viewMap.values()
				.stream()
