...
public class PushNotificationService extends org.iplass.mtp.impl.pushnotification.PushNotificationService {
	...
	private boolean isHttpClientConfigCreated = false;
	...
	public void init(Config config) {
		...
		if (null == httpClientConfig) {
			...
			isHttpClientConfigCreated = true;

		}
		objectMapper = config.getValue("objectMapper", ObjectMapper.class);
			...
			exponentialBackoff.execute(() -> {
				...
				if (PushNotificationStatus.FAIL_RETRYABLE == responseDetail.getStatus()) {
					...
					if (maxElapsedDateTime.isAfter(waitFinishDateTime)) {
						...
						try {
							Thread.sleep(Duration.ofSeconds(responseDetail.getRetryAfterSeconds()).toMillis());
	...
	protected PushNotificationResponseDetail requestPushApi(PushNotificationTarget target, Map<String, Object> messageResource) {
		...
		try {
			...
			logger.debug("Request - requestBody = {}, headers = {}", requestBody, request.getAllHeaders());
			...
			PushNotificationResponseDetail responseDetail = httpClientConfig.getInstance().execute(request, response -> {
				...
				try {
					...
					logger.debug("Response - serviceEndpoint = {}, code = {}({}), contents = {}, headers = {}",
							serviceEndpoint, response.getStatusLine().getStatusCode(), response.getStatusLine().getReasonPhrase(), contents, response.getAllHeaders());
					...
					return handleResponse(target, response.getStatusLine().getStatusCode(), contents, retryAfterSeconds);
	...
	private long toRetryAfterSeconds(String retryAfter, long defaultSeconds) {
		...
		Date now = new Date();
		Date retryAfterDate = DateUtils.parseDate(retryAfter);

		long retryAfterMillis = retryAfterDate.getTime() - now.getTime();
		// マイナス値になる場合は 0 を設定する
		return 0 <= retryAfterMillis ? retryAfterMillis / 1000 : 0;
	}
