...
public class CreateGemUserRoleProcess extends AbstractCreateRoleProcess {
	...
	public boolean execute(TenantCreateParameter param, LogHandler logHandler) {
		...
		//ロール
		Entity role = insertGroupRole("GemUser", em);
		...
		logHandler.info(ToolsResourceBundleUtil.resourceString(param.getLoggerLanguage(), "tenant.create.createdGemUserRole"));
	...
	private void insertEntityPermission(Entity role, EntityManager em) {
		...
		edm.definitionSummaryList("mtp.auth.", true).forEach(ed -> {
			GenericEntity authPermission = createEntityPermission(ed.getName(), role, true, false, false, false);
		...
		//mtp.Information
		//参照のみ
		GenericEntity permission = createEntityPermission("mtp.Information", role, true, false, false, false);
		em.insert(permission);
