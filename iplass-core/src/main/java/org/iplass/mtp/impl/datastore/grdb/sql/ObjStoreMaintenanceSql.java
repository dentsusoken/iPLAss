...
public class ObjStoreMaintenanceSql extends UpdateSqlHandler {
	...
	public String insertNewPage(int tenantId, String metaId, int pageNo, String tableNamePostfix, RdbAdapter rdb) {
		...
		sb.append(ObjStoreTable.OBJ_VER);
		sb.append(" FROM ")
				.append(MetaGRdbEntityStore.makeObjStoreName(tableNamePostfix));
		sb.append(" WHERE ");
		sb.append(ObjStoreTable.TENANT_ID)
				.append("=")
				.append(tenantId);
	...
	public String insertNewPageRB(int tenantId, String metaId, int pageNo, String tableNamePostfix, RdbAdapter rdb) {
		...
		sb.append("(");
		sb.append(ObjStoreTable.RB_ID)
				.append(",");
		sb.append(ObjStoreTable.RB_DATE)
				.append(",");
		sb.append(ObjStoreTable.RB_USER)
				.append(",");
		sb.append(ObjStoreTable.TENANT_ID)
				.append(",");
		sb.append(ObjStoreTable.OBJ_DEF_ID)
		...
		sb.append(ObjStoreTable.OBJ_VER);
		sb.append(" FROM ")
				.append(MetaGRdbEntityStore.makeObjRbTableName(tableNamePostfix));
		sb.append(" WHERE ");
		sb.append(ObjStoreTable.TENANT_ID)
				.append("=")
				.append(tenantId);
	...
	private String updateColInternal(int tenantId, String metaId, int defVer, int pageNo, List<ColCopy> ccl, String tableName, boolean isRB,
			RdbAdapter rdb) {
		StringBuilder sb = new StringBuilder();
			...
			for (int i = 0; i < ccl.size(); i++) {
				...
				if (cc.isSetToNull()) {
					...
					if (cc.getIndexType() == null) {
						//通常カラム
						sb.append(cc.getToColName())
								.append("=null");
					} else {
						//Indexカラム
						sb.append(cc.getToColName() + ObjStoreTable.INDEX_TD_POSTFIX)
								.append("=null");
						sb.append(",");
						sb.append(cc.getToColName())
								.append("=null");
					...
					if (cc.getIndexType() == null) {
						...
						sb.append(cc.getToColName())
								.append("=");
						cc.getConverter()
								.appendConvertExp(sb, cc.getFromColName(), rdb);
//						if (cc.isNeedCast()) {
...
//							sb.append(cc.getFromColName());
	...
	private String updateColDirectJoinInternal(int tenantId, String metaId, int defVer, List<ColCopy>[] ccls, String tableName, boolean isRB,
			RdbAdapter rdb) {
		StringBuilder sb = new StringBuilder();
		if (rdb.isNeedFromClauseTableAliasUpdateStatement()) {
			sb.append("UPDATE ")
	...
	private void multitableJoinClause(StringBuilder sb, int tenantId, String metaId, int defVer, List<ColCopy>[] ccls, String tableName, boolean isRB,
			RdbAdapter rdb) {
				...
				for (ColCopy cc : ccls[i]) {
					...
					if (cc.isSetToNull()) {
						...
						if (cc.getIndexType() == null) {
							//通常カラム
							sb.append("p")
									.append(cc.getToPageNo())
									.append(".")
									.append(cc.getToColName())
									.append("=null");
						} else {
							//Indexカラム
							sb.append("p")
									.append(cc.getToPageNo())
									.append(".")
									.append(cc.getToColName() + ObjStoreTable.INDEX_TD_POSTFIX)
									.append("=null");
							sb.append(",");
							sb.append("p")
									.append(cc.getToPageNo())
						...
						if (cc.getIndexType() == null) {
							...
							sb.append("p")
									...
									.append("=");
							cc.getConverter()
									.appendConvertExp(sb, "p" + cc.getFromPageNo() + "." + cc.getFromColName(), rdb);
						} else {
							//Indexカラム
							sb.append("p")
									.append(cc.getToPageNo())
