...
public class DefaultPropertyValueCoder implements PropertyValueCoder {
	...
	public void open(Properties prop) {
		try {
			...
			try (PassphraseSupplier passphraseSupplier = (PassphraseSupplier) Class.forName(passphraseSupplierName)
					.newInstance()) {
				...
				try {
					SecretKey secKey = SecretKeyFactory.getInstance(keyAlg)
