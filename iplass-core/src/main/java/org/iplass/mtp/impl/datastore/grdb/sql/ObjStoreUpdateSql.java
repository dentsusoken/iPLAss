...
public class ObjStoreUpdateSql extends UpdateSqlHandler {
	...
	private boolean handlePropAndVal(StringBuilder sb, int tenantId, EntityHandler eh, Entity model, List<String> updatePropNames, int pageNo,
			...
			if (canUpdateProperty(pHandler)) {
				...
				for (int i = 0; i < cols.size(); i++) {
					...
					if (tcol.getMetaData()
						...
						sb.append(tcol.getMetaData()
								.getColumnName());
						sb.append("=");
						if (col.isNative()) {
							col.getSingleColumnRdbTypeAdapter()
									.appendToSqlAsRealType(valToSet, sb, rdbAdaptor);
						} else {
							col.getSingleColumnRdbTypeAdapter()
					...
					//index col
					if (tcol.getIndexColName() != null && tcol.getMetaData()
						...
						case UNIQUE:
							...
							sb.append("=");
							sb.append("'");
							sb.append(MetaGRdbPropertyStore.makeInternalIndexKey(tenantId, rdbAdaptor.sanitize(pHandler.getParent()
									.getMetaData()
									.getId()), pageNo));
							sb.append("',");
							sb.append(tcol.getIndexColName());
	...
	public String updateMain(int tenantId, EntityHandler eh,
		...
		sb.append(" AND " + ObjStoreTable.OBJ_DEF_VER + "<=")
				.append(((MetaGRdbEntityStore) eh.getEntityStoreRuntime()
						.getMetaData()).getVersion());
		if (withTimestampCheck) {
			sb.append(" AND " + ObjStoreTable.UP_DATE + "=")
					.append(rdbAdaptor.toTimeStampExpression(updateDate));
		}
	...
	private String valueExp(ValueExpression val, EntityHandler eh, GRdbPropertyStoreRuntime col, int pageNo, RdbAdapter rdbAdaptor,
		...
		if (val == null) {
			...
			if (literalValue == null) {
				...
				if (col.isNative()) {
					col.getSingleColumnRdbTypeAdapter()
							.appendToSqlAsRealType(literalValue, vsb, rdbAdaptor);
				} else {
					col.getSingleColumnRdbTypeAdapter()
							.appendToTypedCol(vsb, rdbAdaptor,
									() -> col.getSingleColumnRdbTypeAdapter()
											.appendToSqlAsRealType(literalValue, vsb, rdbAdaptor));
				}
				valStr = vsb.toString();
			...
			if (chk.hasOtherPage && rdbAdaptor.getMultiTableUpdateMethod() != MultiTableUpdateMethod.DIRECT_JOIN) {
				...
				//index利用しないように
				q.select()
						.addHint(new IndexHint(Entity.NAME));
				q.accept(conv);
				StringBuilder vsb = new StringBuilder();
				vsb.append("(");
				...
				vsb.append(" AND ")
						.append(mainTable)
						.append("." + ObjStoreTable.OBJ_ID + "=u." + ObjStoreTable.OBJ_ID);
				vsb.append(" AND ")
						.append(mainTable)
						.append("." + ObjStoreTable.OBJ_VER + "=u." + ObjStoreTable.OBJ_VER);
				vsb.append(")");
				...
				if (col.isNative()) {
					...
					col.getSingleColumnRdbTypeAdapter()
							.appendToTypedCol(vsb2, rdbAdaptor,
									() -> vsb2.append(vsb));
				...
				val.accept(conv);
				if (col.isNative()) {
					valStr = qc.getCurrentSb()
							.toString();
				} else {
					StringBuilder vsb = new StringBuilder();
					col.getSingleColumnRdbTypeAdapter()
							.appendToTypedCol(vsb, rdbAdaptor,
									() -> vsb.append(qc.getCurrentSb()
											.toString()));
					valStr = vsb.toString();
				}
			}
	...
	private boolean handlePropAndValforUpdateAll(int tenantId, StringBuilder sb, EntityHandler eh, List<UpdateValue> values, int pageNo,
			RdbAdapter rdbAdaptor, EntityContext context) {
		boolean added = false;
		...
		for (UpdateValue uv : values) {
			...
			if (canUpdateProperty(pHandler)) {
				...
				if (col.isMulti() && uv.getPropertyName()
						.getArrayIndex() != EntityField.ARRAY_INDEX_UNSPECIFIED) {
					if (uv.getPropertyName()
							.getArrayIndex() < 0
							|| uv.getPropertyName()
									.getArrayIndex() >= cols.size()) {
						throw new QueryException(
								"array index out of range. property=" + pHandler.getName() + ", index=" + uv.getPropertyName()
										.getArrayIndex());
					}
					cols = cols.get(uv.getPropertyName()
							.getArrayIndex())
							.asList();
				}
				...
				for (int i = 0; i < cols.size(); i++) {
					...
					//index col
					if (!tcol.isExternalIndex() && tcol.getIndexColName() != null && tcol.getMetaData()
							.getIndexPageNo() == pageNo) {
						if (added) {
							sb.append(",");
						} else {
	...
	private void multitableJoinClause(StringBuilder sb, int tenantId, int pageNo, EntityHandler eh, EntityContext ec, List<UpdateValue> values,
	...
	public String updateAllSubByTempTable(int tenantId, EntityHandler eh,
		...
		if (rdbAdaptor.isNeedFromClauseTableAliasUpdateStatement()) {
			...
			sb.append("UPDATE ")
					.append(((GRdbEntityStoreRuntime) eh.getEntityStoreRuntime()).OBJ_STORE())
					.append(" ")
					.append(mainTable);
		}

		if (rdbAdaptor.getMultiTableUpdateMethod() == MultiTableUpdateMethod.DIRECT_JOIN) {
			multitableJoinClause(sb, tenantId, pageNo, eh, context, values, ((GRdbEntityStoreRuntime) eh.getEntityStoreRuntime()).OBJ_STORE(),
					rdbAdaptor);
		}
		...
		if (rdbAdaptor.isSupportRowValueConstructor()) {
			sb.append(" AND (")
					...
					.append(mainTable)
					.append(".")
					.append(ObjStoreTable.OBJ_VER + ") IN("
							+ "SELECT " + ObjStoreTable.OBJ_ID + "," + ObjStoreTable.OBJ_VER + " FROM " + rdbAdaptor.getTemplaryTablePrefix()
	...
	public String updateAllMainByTempTable(int tenantId, EntityHandler eh,
		...
		sb.append(ObjStoreTable.UP_USER + "='")
				.append(rdbAdaptor.sanitize(userId))
				.append("',");
		if (rdbAdaptor.getMultiTableUpdateMethod() == MultiTableUpdateMethod.DIRECT_JOIN) {
			sb.append(mainTable)
					.append(".");
		...
		sb.append(ObjStoreTable.UP_DATE + "=")
				.append(rdbAdaptor.systimestamp());
		...
		sb.append(" AND ")
				.append(mainTable)
				.append("." + ObjStoreTable.OBJ_DEF_VER + "<=")
				.append(((MetaGRdbEntityStore) eh.getEntityStoreRuntime()
						.getMetaData()).getVersion());
	...
	private static class HasOtherPageChecker extends QueryVisitorSupport {
		...
		public boolean visit(EntityField entityField) {
			if (!hasOtherPage) {
				...
				if (ph == null) {
					throw new QueryException(entityField.getPropertyName() + " not define on" + eh.getMetaData()
							.getName());
				}
	...
	private static class NeedOtherPageJoinChecker extends QueryVisitorSupport {
		...
		public boolean visit(EntityField entityField) {
			...
			if (col.isMulti() && entityField.getArrayIndex() != EntityField.ARRAY_INDEX_UNSPECIFIED) {
				...
				cols = cols.get(entityField.getArrayIndex())
						.asList();
			}
			for (GRdbPropertyStoreHandler c : cols) {
				if (c.getMetaData()
					...
					otherPn.add(c.getMetaData()
							.getPageNo());
				}
				if (!c.isExternalIndex() && c.getIndexColName() != null && c.getMetaData()
						.getIndexPageNo() != myPageNo) {
					if (otherPn == null) {
						otherPn = new HashSet<>();
					}
					otherPn.add(c.getMetaData()
