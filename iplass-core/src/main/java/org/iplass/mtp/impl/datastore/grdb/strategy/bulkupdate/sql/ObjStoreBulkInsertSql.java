...
public class ObjStoreBulkInsertSql {
	...
	public static List<ColumnValueMapper> insertSubPage(int tenantId, EntityHandler eh,
		...
		ret.add(new FixedExpressionColumnValueMapper(ObjStoreTable.OBJ_DEF_ID, rdb.toSqlExp(eh.getMetaData()
				.getId())));
	...
	private static void handlePropList(List<ColumnValueMapper> mappers, int tenantId, List<PropertyHandler> propList, EntityHandler eh, int pageNo,
			RdbAdapter rdbAdaptor) {
	...
	public static List<ColumnValueMapper> insertMain(int tenantId, EntityHandler eh,
		...
		ret.add(new FixedExpressionColumnValueMapper(ObjStoreTable.OBJ_DEF_VER, Integer.toString(((MetaGRdbEntityStore) eh.getMetaData()
