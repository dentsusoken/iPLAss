...
public class EntityService extends AbstractTypedMetaDataService<MetaEntity, EntityHandler> implements Service {
	...
	public VersionController getVersionController(EntityHandler eh) {
		if (eh.getMetaData()
				.getVersionControlType() == null) {
			return versionControllers.get(VersionControlType.NONE);
		}
		return versionControllers.get(eh.getMetaData()
	...
	private String doCreate(final MetaEntity newMeta, final MetaDataConfig config, final EntityContext context, boolean doAutoReload) {
		...
		List<MetaDataEntryInfo> infoList = ServiceRegistry.getRegistry()
				.getService(MetaDataRepository.class)
				.getHistoryById(ExecuteContext.getCurrentContext()
						.getClientTenantId(), newMeta.getId());
		if (infoList != null) {
		...
		if (curerntMaxVersion >= 0) {
			...
			MetaDataEntry curEntry = MetaDataContext.getContext()
					.getMetaDataEntryById(newMeta.getId(), curerntMaxVersion);
		...
		if (prevMeta == null) {
			...
			MetaDataContext.getContext()
					.store(path, newMeta, /* curerntMaxVersion + 1, */ config, doAutoReload);
			...
			if (!prepare) {
				throw new EntityRuntimeException(
						"can not prepare for re-create Entity:" + newMeta.getName() + "(tenant=" + ExecuteContext.getCurrentContext()
								.getClientTenantId() + ")");
			}
			...
			try {
				res = updateData(configDataModel, ExecuteContext.getCurrentContext()
						.getClientTenantId(), newMeta, prevMeta, context, config, curerntMaxVersion);
				if (res) {
					//MetaData登録
					MetaDataContext.getContext()
							.store(path, newMeta, /* curerntMaxVersion + 1, */ config, doAutoReload);
					...
					throw new EntityRuntimeException(
							"can not re-create Entity Definition:" + newMeta.getName() + "(tenant=" + ExecuteContext.getCurrentContext()
									.getClientTenantId() + ")");
				}
	...
	private boolean updateData(final ApplyMetaDataStrategy st, final int tenantId, final MetaEntity newOne, final MetaEntity previous,
		...
		if (previousVersion == null) {
			ent = MetaDataContext.getContext()
					.getMetaDataEntryById(previous.getId());
		} else {
			ent = MetaDataContext.getContext()
					.getMetaDataEntryById(previous.getId(), previousVersion);
		}
		if (tcService.getSharedTenantId() == tenantId
				&& ent.isSharable()
				&& !ent.isDataSharable()) {
			List<Integer> allTenantIds = tcService.getAllTenantIdList();
			List<Integer> overwritedTenantIds = MetaDataContext.getContext()
					.getOverwriteTenantIdList(previous.getId());
			for (Iterator<Integer> it = allTenantIds.iterator(); it.hasNext();) {
				Integer id = it.next();
				if (overwritedTenantIds.contains(id)) {
					logger.info(id + "'s MetaData:" + ent.getPath() + " (" + ent.getMetaData()
							.getId() + ") is overwrote. so skip data patch.");
					it.remove();
				}
			}
...
//					newOne.createRuntime(new MetaDataConfig(ent.isSharable(), ent.isOverwritable(), ent.isDataSharable()));
			...
			tc.getMetaDataContext()
					.update(convertPath(ENTITY_META_PATH + newOne.getName()), newOne);

			if (config != null) {
				tc.getMetaDataContext()
						.updateConfig(convertPath(ENTITY_META_PATH + newOne.getName()), config);
			}
	...
	private boolean doRemove(final ApplyMetaDataStrategy st, final int tenantId, final MetaEntity previous, final EntityContext context) {
		return Transaction.requiresNew(t -> {
			TenantContext tc = ServiceRegistry.getRegistry()
					...
					.getTenantContext(tenantId);
			tc.getMetaDataContext()
					.remove(convertPath(ENTITY_META_PATH + previous.getName()));//, previous);
			return true;
		});
	}
	...
	private boolean doDefragMeta(final ApplyMetaDataStrategy st, final int tenantId, final MetaEntity target, final EntityContext context) {
		return Transaction.requiresNew(transaction -> {
			...
			MetaDataEntry ent = MetaDataContext.getContext()
					.getMetaDataEntryById(target.getId());
			...
			if (tcService.getSharedTenantId() == tenantId
				...
				List<Integer> allTenantIds = tcService.getAllTenantIdList();
				List<Integer> overwritedTenantIds = MetaDataContext.getContext()
						.getOverwriteTenantIdList(target.getId());
				for (Iterator<Integer> it = allTenantIds.iterator(); it.hasNext();) {
					Integer id = it.next();
					if (overwritedTenantIds.contains(id)) {
						logger.info(id + "'s MetaData:" + ent.getPath() + " (" + ent.getMetaData()
								.getId() + ") is overwrote. so skip defrag data.");
						it.remove();
					}
				}
				targetTenantIds = new int[allTenantIds.size()];
			...
			if (!before.equals(target)) {
				TenantContext tc = ServiceRegistry.getRegistry()
						...
						.getTenantContext(tenantId);
				tc.getMetaDataContext()
						.update(convertPath(ENTITY_META_PATH + target.getName()), target);
			}
	...
	public Future<String> updateDataModelSchema(final EntityDefinition definition, Map<String, String> renamePropertyMap) {
		...
		MetaEntity newMeta = currentHandler.getMetaData()
				.copy();
...
//					ExecuteContext.setContext(currentContext);
				...
				if (newMeta.getEntityStoreDefinition() != null &&
						!newMeta.getEntityStoreDefinition()
								.getClass()
								.equals(currentDs.getEntityStoreType())) {
					if (currentHandler.getMetaData()
							.getStoreMapping() != null) {
						newMeta.setStoreMapping((MetaStoreMapping) currentHandler.getMetaData()
								.getStoreMapping()
								.copy());
					}
				}

				logger.info("update " + currentHandler.getMetaData()
						.getName() + "("
						+ currentHandler.getMetaData()
								.getId()
						+ ") process start");
				boolean prepare = doPrepare(reConfigDataModel, currentContext.getClientTenantId(), newMeta, currentHandler.getMetaData(), context);

				if (!prepare) {
					throw new EntityRuntimeException("can not prepare for update Entity:" + currentHandler.getMetaData()
							.getName() + "(tenant=" + currentContext.getClientTenantId() + ")");
				}
				...
				try {
					...
					if (result) {
						...
						throw new EntityRuntimeException("can not modify Entity Definition:" + currentHandler.getMetaData()
								.getName() + "(tenant=" + currentContext.getClientTenantId() + ")");
					}
				} finally {
...
//					ExecuteContext.setContext(currentContext);
				...
				ApplyMetaDataStrategy reConfigDataModel = currentHandler.getDataStore()
						.getApplyMetaDataStrategy();
				MetaEntity metaEntity = currentHandler.getMetaData();

				logger.info("remove " + currentHandler.getMetaData()
						.getName() + "("
						+ currentHandler.getMetaData()
								.getId()
						+ ") process start");
				...
				if (!prepare) {
					throw new EntityRuntimeException("can not prepare for remove Entity:" + currentHandler.getMetaData()
							.getName() + "(tenant=" + currentContext.getClientTenantId() + ")");
				}
				boolean result = false;
				...
				try {
					MetaDataEntry ent = MetaDataContext.getContext()
							.getMetaDataEntryById(metaEntity.getId());
					if (ent.getRepositryType() == RepositoryType.SHARED_OVERWRITE) {
						//上書きされているので、SHAREDの初期状態で再上書き
						MetaDataContext shredContext = ServiceRegistry.getRegistry()
								.getService(TenantContextService.class)
								.getSharedTenantContext()
								.getMetaDataContext();
					...
					if (!result) {
						throw new EntityRuntimeException("can not delete Entity definition:" + currentHandler.getMetaData()
								.getName() + "(tenant=" + currentContext.getClientTenantId() + ")");
					}
...
//				}
	...
	public List<MetaDataEntryInfo> list(String path) {
		List<MetaDataEntryInfo> list = MetaDataContext.getContext()
				.definitionList(convertPath(getFixedPath() + path));
		if (list != null) {
			//Entityの除外
	...
	public EntityHandler getRuntimeByName(String name) {
		return MetaDataContext.getContext()
				.getMetaDataHandler(EntityHandler.class, convertPath(ENTITY_META_PATH + name));
	}

	public EntityHandler getRuntimeById(String id) {
		return MetaDataContext.getContext()
				.getMetaDataHandlerById(EntityHandler.class, id);
	}

	public EntityHandler getHandlerById(String id, int version) {
		return MetaDataContext.getContext()
				.getMetaDataHandlerById(EntityHandler.class, id, version);
	}
	...
	public void checkValidOidPattern(String oid) {
		if (oidValidationPattern != null) {
			if (!oidValidationPattern.matcher(oid)
					.matches()) {
