...
//		transactionLocalCacheByName = new HashMap<String, EntityHandler>();
	...
	public static EntityContext getCurrentContext() {
		...
		TransactionManager tm = ServiceRegistry.getRegistry()
		...
		if (tm != null) {
			...
			if (t != null && t.getStatus() == TransactionStatus.ACTIVE) {
				...
				if (metaContext == null) {
					int sharedTenantId = ServiceRegistry.getRegistry()
							.getService(TenantContextService.class)
							.getSharedTenantId();
					metaContext = new EntityContext(tenantContext.getTenantId(), sharedTenantId, ServiceRegistry.getRegistry()
		...
		int sharedTenantId = ServiceRegistry.getRegistry()
				.getService(TenantContextService.class)
				.getSharedTenantId();
		return new EntityContext(tenantContext.getTenantId(), sharedTenantId, ServiceRegistry.getRegistry()
				.getService(EntityService.class));
	}
}
