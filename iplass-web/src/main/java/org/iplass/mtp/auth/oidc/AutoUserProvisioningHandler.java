 ...
 * iPLAss内に存在しないユーザを自動的に作成する場合に実装するインタフェース。
 * 
 * @author K.Higuchi
 *
 */
public interface AutoUserProvisioningHandler {
	...
	/**
	 * 認証されたユーザが存在しない場合に呼び出されるので、
	 * Userエンティティを生成する処理を記述する。
	 * 
	 * @param subjectId OpenID Provider内で一意となるユーザ識別子
	...
	/**
	 * 認証されたユーザの再ログインのタイミングで、呼び出される。
	 * ユーザ属性を更新する必要がある場合は、更新処理を記述する。
	 * 
	 * @param user
	 * @param subjectId OpenID Provider内で一意となるユーザ識別子
	 * @param subjectName OpenId Connect定義で指定されるsubjectNameClaimの値
	 ...
	 * <table border="1">
	 * <tr><th>Userエンティティのプロパティ</th><th>OIDCで連携されたユーザ属性</th></tr>
