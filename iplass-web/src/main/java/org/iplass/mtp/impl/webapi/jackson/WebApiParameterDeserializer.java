...
public class WebApiParameterDeserializer extends JsonDeserializer<WebApiParameter> {
	...
	// @SuppressWarnings("resource") ⇒ *1, *2 の位置で 引数 JsonParser jp のクローズをしていないという警告を抑制。
	...
	public WebApiParameter deserialize(JsonParser jp, DeserializationContext ctxt)
		...
		for (; jp.getCurrentToken() != JsonToken.END_OBJECT; jp.nextToken()) {
			...
			if (NAME_PROPERTY_NAME.equals(propName)) {
				...
				if (name == null) {
					// *1
					throw JsonMappingException.from(ctxt.getParser(), "name is null.");
				}
				param.setName(name);
				...
				if (valType == null) {
					// *2
					throw JsonMappingException.from(ctxt.getParser(), "valueType is null.");
				}
	...
	private Object readValue(String valueType, JsonParser jp, DeserializationContext ctxt) throws IOException, JsonProcessingException {
		...
		if (cl == null) {
			throw JsonMappingException.from(ctxt.getParser(), "unknown valueType:" + valueType);
		}
