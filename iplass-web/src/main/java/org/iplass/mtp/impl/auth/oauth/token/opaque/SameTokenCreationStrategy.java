...
public class SameTokenCreationStrategy implements TokenCreationStrategy {
	...
	public AuthToken create(OAuthClientRuntime client, AccessTokenHandler handler, String userUniqueId, AccessTokenInfo accessTokenInfo) {
		...
		if (handler.getHashSettings() != null && handler.getHashSettings()
				.size() > 0) {
		...
		for (int i = 0; i <= retryCount; i++) {
			...
			try {
				at = Transaction.requiresNew(t -> {
					...
					if (atm.getGrantedScopes() == null || !atm.getGrantedScopes()
							.containsAll(accessTokenInfo.getGrantedScopes())) {
					...
					if (accessTokenInfo.getGrantedScopes()
						...
						if (authorizationServer.getClientPolicy(client.getMetaData()
								.getClientType())
								.isRequireRefreshToken(accessTokenInfo.getGrantedScopes())) {
							needRefreshToken = true;
						}
					}

					if (needRefreshToken && retTokenMement.getRefreshToken() == null) {
						retTokenMement.setRefreshToken(handler.refreshTokenHandler()
								.authTokenStore()
								.getBySeries(tenantId, handler.refreshTokenHandler()
										.getType(), retToken.getSeries()));
					}

					if (needRefreshToken) {
						if (retTokenMement.getRefreshToken() == null
								|| ((RefreshTokenMement) retTokenMement.getRefreshToken()
										.getDetails()).getExpires() <= System.currentTimeMillis()) {
							handler.refreshTokenHandler()
									.authTokenStore()
									.deleteBySeries(tenantId, handler.refreshTokenHandler()
											.getType(), retToken.getSeries());
							AuthToken refreshToken = handler.refreshTokenHandler()
									.newAuthToken(retToken.getOwnerId(), retToken.getPolicyName(), new RefreshTokenInfo(client.getMetaData()
											.getName()));
							handler.refreshTokenHandler()
									.authTokenStore()
									.create(refreshToken);
