		...
		for (int i = 0; i <= retryCount; i++) {
			...
			try {
				at = Transaction.requiresNew(t -> {
					...
					if (null != retToken) {
						logger.debug(
								"=====  check variable 0. retToken.getType() = {}, retToken.getOwnerId() = {}, retToken.getSeries() = {}, retToken.getToken() = {}",
								retToken.getType(), retToken.getOwnerId(), retToken.getSeries(), retToken.getToken());
					}
					...
					logger.debug("===== -- check variable 2. needRefreshToken = {}, retTokenMement.getRefreshToken() = {}", needRefreshToken,
							retTokenMement.getRefreshToken());

					if (needRefreshToken) {

						long currentTimeMillis = System.currentTimeMillis();

						logger.debug("===== --- check variable 3. retTokenMement.getRefreshToken() = {}", retTokenMement.getRefreshToken());
						if (null != retTokenMement.getRefreshToken()) {
							logger.debug(
									"===== ---- check variable 4. ((RefreshTokenMement) retTokenMement.getRefreshToken().getDetails()).getExpires() = {}, currentTimeMillis = {}",
						...
						logger.debug("===== ----- check variable 5. retTokenMement.getRefreshToken() = {}", retTokenMement.getRefreshToken());
					}

					if (null != retToken) {
						logger.debug(
								"===== ------ check variable 6. retToken.getType() = {}, retToken.getOwnerId() = {}, retToken.getSeries() = {}, retToken.getToken() = {}",
								retToken.getType(), retToken.getOwnerId(), retToken.getSeries(), retToken.getToken());
					}
